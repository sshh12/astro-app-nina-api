...
using Plugin.NINA.AstroAppHTTPAPI.Properties;
using System;
...
namespace Plugin.NINA.AstroAppHTTPAPI {
    ...
    /// The user interface for the settings will be defined by a DataTemplate with the key having the naming convention "AstroAppHttpApi_Options" where AstroAppHttpApi corresponds to the AssemblyTitle - In this template example it is found in the Options.xaml
    /// </summary>
    ...
    public class AstroAppHttpApi : PluginBase, INotifyPropertyChanged {
        ...
        private readonly ImagePattern exampleImagePattern = new ImagePattern("$$EXAMPLEPATTERN$$", "An example of an image pattern implementation", "Astro App HTTP API");

        [ImportingConstructor]
        public AstroAppHttpApi(IProfileService profileService, IOptionsVM options, IImageSaveMediator imageSaveMediator) {
